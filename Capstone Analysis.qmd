...
data<-read.csv(file.choose(),strings=T) #pop-up window allows you to double-click file
##data <- read.csv("synthetic_fraud_dataset.csv", stringsAsFactors = FALSE)
...
p_cm <- ggplot(cm_table, aes(x = Ref, y = Pred, fill = Freq)) +
  ...
  coord_fixed()
print(p_cm)
ggsave("Confusion_Matrix.png", p_cm, width = 8, height = 6, dpi = 300, bg = "white")
cat("\nConfusion Matrix plot saved as 'Confusion_Matrix.png'\n")

# --- 10. Optional: Print Full Confusion Table ---
...
```
This confusion matrix shows that the model correctly identified 2,318 fraudulent transactions (true positives) and 10,102 legitimate ones (true negatives), while misclassifying 2,502 frauds as non-fraud (false negatives) and incorrectly flagging 77 legitimate transactions as fraud (false positives). With an overall accuracy of 82.8%, the model performs significantly better than random guessing (No Information Rate = 67.9%, p < 2.2e-16). The sensitivity of 90.2% indicates strong ability to detect fraud, while the specificity of 99.6% shows excellent performance in recognizing legitimate transactions. The balanced accuracy of 73.7% confirms the model handles both classes reasonably well despite class imbalance, and the Kappa score of 0.5466 reflects moderate agreement between predicted and actual labels.
...
gam_mod <- gam(
  Fraud_Label ~
    s(Risk_Score, k = 20) +                  
    s(Transaction_Amount, k = 20) +
    s(Transaction_Distance, k = 20) +
    Previous_Fraudulent_Activity +
    Device_Type +
    Card_Type +
    Is_Weekend,
