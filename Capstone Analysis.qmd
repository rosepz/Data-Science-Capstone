...
install.packages(c("mgcv", "pROC", "caret", "dplyr", "ggplot2"))
library(mgcv)
library(pROC)
library(caret)
library(dplyr)
...
# Load the prepared dataset (Ensure 'synthetic_fraud_gam_data.csv' is in your working directory)
data <- read.csv("synthetic_fraud_gam_data.csv")
...
# ------------------------------------------------
# 3. Data Splitting (CRITICAL STEP for 'train_data')
# ------------------------------------------------
...
# ------------------------------------------------
# 4. GAM Fitting (Corrected with k=5)
# ------------------------------------------------

# Set k=5 to avoid the "fewer unique covariate combinations" error.
gam_model <- gam(
    Fraud_Label ~ s(Transaction_Amount, k=5) +
                  s(Account_Balance, k=5) +
                  s(Risk_Score, k=5) +
                  s(Transaction_Distance, k=5) +
                  ...
                  Device_Type +
                  Card_Type,
    data = train_data, # This now exists!
...
# ------------------------------------------------
# 7. Plotting the ROC Curve (Image Generation)
# ------------------------------------------------

# This saves the ROC curve to a PNG file named 'gam_roc_curve_final.png'
png("gam_roc_curve_final.png", width = 800, height = 600, res = 100)
plot(roc_obj, main = paste("GAM Model ROC Curve (AUC =", format(round(auc_value, 4), nsmall = 4), ")"),
     col = "#1C78C0", lwd = 2)
abline(a = 1, b = 1, lty = 2, col = "gray50") # Reference line (AUC = 0.5)
dev.off()
